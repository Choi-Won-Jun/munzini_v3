...
func makeSQSResult(qData question.QData, userID string) string { // SQSProbPattern이 NULL이 아닌 경우, 간단문진 결과 출
	...
	if len(qData.SQSProbPatternIdx) >= question.SERIOUS_SQS { // 간단문진 결과 발생한 문제가 SERIOUS_SQS개 이상일 시
		...
		if isDataENOUGH == false {
			...
			sqsResult = "문진 결과를 알려드릴께요." + DB.GetResult_and_Curation(DB.COMPLECATION) + "그럼, 더 자세한 건강상태 확인을 위해 추가 문진을 시작해 볼까요?!"

		} else {
			sqsResult = "문진 결과를 알려드릴께요." + DB.GetResult_and_Curation(DB.COMPLECATION) + recentCKU_result + "그럼, 더 자세한 건강상태 확인을 위해 추가 문진을 시작해 볼까요?!"
		}
	...
	sqsResult = "문진 결과를 알려드릴께요. " + DB.GetResult_and_Curation(identifier) + " 그럼, 더 자세한 건강상태 확인을 위해 추가 문진을 시작해 볼까요?!!!"
	// switch identifier {
	// case "칠정":
