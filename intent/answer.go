...
func makeRecentCheckUPResult(userID string, patterns []string) (string, bool) {
	...
	if flag == false { // DB에 세번 이상의 문진기록이 저장되어있지 않는 경우
		...
		if patterns[0] == DB.COMPLECATION { //현재 진행중인 문진을 통한 진단결과가 미병의심(3 가지 이상 패턴의 조합)인 경우
			...
			year_of_Record := strconv.Itoa(year_of_Record)
			month_of_Record := strconv.Itoa(int(month_of_Record))
			day_of_Record := strconv.Itoa(day_of_Record)
			log.Println("*********************")
			log.Println(year_of_Record)
			log.Println("*********************")
			log.Println(month_of_Record)
			log.Println("*********************")
			log.Println(day_of_Record)
			log.Println("*********************")
